...
\end{equation}
Esto significa que el estado de máxima entropía es \ddnote*{factorizable. Ojo aquí: para estados mixtos, separabilidad no implica factorizabilidad, pero factorizabilidad si implica separabilidad. Tienes razón con que son separables, pero en realidad tienen una propiedad mas fuerte: son factorizables.}{separable} y tiene exactamente $n$ factores. Explícitamente,
...
\end{equation}\ddnote{acuerdame de platicar, hay una forma trivial de sacar esta ecuación}
...
\end{equation}
Con estas dos ecuaciones deducimos la relación entre \ddnote*{los promedios de las observables a nivel grueso}{las cantidades medibles} $\expval{\pauli{j}}=r_{\ef}(\hat{r}_{\ef})_{j}$ y los multiplicadores de Lagrange introducidos para la maximización de la entropía, y es
\begin{equation}
...
\end{gather}
donde la dependencia de la asignación en el modelo de grano grueso se indica mediante un subíndice\ddnote{, $\mcC$, ver ecuación *tal*}. Nótese que según los valores que puedan tomar las diferentes probabilidades, el estado asignado tendrá diferentes propiedades. \ddnote*{Esto tiene varios problemas, das dos casos, pero el segundo no es cierto, no es lineal. Después los explicas en el orden opuesto.}{En dos casos particulares, la aplicación de asignación es lineal: cuando $p_{1}=1$ y cuando $p_{j}=\frac{1}{n}\forall j$.
