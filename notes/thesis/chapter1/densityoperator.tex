...
A este sistema se le llama ``mezlca estadística''\ddnote{En este caso se dice que el estado del sistema es una ``mexcla estadística''}, y no debe confundirse con una superposición de estados $\ket{\varphi_{i}}$ con coeficientes $\sqrt{p_{i}}$, ya que una superposición está bien caracterizada, y está completamente descrita por $\ket{\psi}=\sum_{i}\sqrt{p_{i}}\ket{\varphi_{i}}$, mientras que la mezcla no lo está: el elemento probabilístico está asociado a un grado de ignorancia sobre la preparación del sistema. La mezcla estadísitica, en este sentido, toma en cuenta no sólo la probabilidad intrínseca a cada estado cuántico, sino una probabilidad más \ddnote{este más está de más} clásica, $p_{i}$. Consideremos ahora una observable descrita por un operador hermítico $A$, se sabe que el valor de expectación del observable, con respecto a un estado $\ket{\varphi_{i}}$, está dado por $\expval{A}=\bra{\varphi_{i}}A\ket{\varphi_{i}}$ \ddnote{cambié la redacción de la frase anterior}. El valor esperado de dicho observable con respecto a la mezcla estadística será, justamente, la combinación probabilística de los valores esperados respecto a los elementos de la mezcla:
\begin{equation*}
...
\subsection{Sistemas compuestos}\label{sec:Ch1PartialTrace}
\ddnote{titulo mejor: sistemas multipartitos}
