...
\end{align*}
\ddnote{Esta derivación es correcta y está bien dejarla así. Solo te dejo un trucazo: la traza de un número es el mismo número (son isomorfos a las matrices de $1\times 1$), entonces $\bra{\phi}A\ket{\phi}=\tr (\bra{\phi}A\ket{\phi})=\tr (A \ket{\phi}\bra{\phi})$ (usando la propiedad cíclica de la traza)}
...
\end{equation}
y vemos que es posible hallar el valor esperado de un observable respecto a un sistema\ddnote{he visto varias veces que intercambias estado por sistema, ten cuidado, checa en todos lados donde usas sistema a ver si lo usas correctamente} a través del operador de densidad que lo describe mediante
...
\end{equation*}
\ddnote{Es mas directo y claro si mejor comparas el caso cuántico con el caso clásico discreto, pues el caso cuántico que usas es también discreto. Si quieres mencionar el continuo está bien, pero que sea secundario.}
Si se asocia la operación traza (la suma sobre los elementos diagonales de la matriz $A$) con la integral (la suma continua sobre los valores de $A(x)$)\ddnote{esta explicación tiene problemas, es por eso que es mejor usar el caso discreto. Que es una suma continua? (usar heuristica no es malo, solo hay q decir q es heuristica o algún sinínimo, pero mejor usa el caso discreto)}, es posible ver que la matriz de densidad ocupa un rol similar al de la función de densidad.
...
\end{enumerate}
La primera propiedad se deriva de la normalización de los estados $\ket{\varphi_{i}}$ que definen al sistema. \ddnote{Edité la siguiente línea:}{\blue La segunda establece que $\rho$ es una matriz positiva semidefinida y puede interpretarse [...]} La segunda, llamada la semidefinición positiva de $\rho$, puede interpretarse como la necesidad de que la probabilidad de que el sistema descrito por $\rho$ se halle en el estado $\dyad{\varphi}$ sea mayor o igual a $0$. Un operador es un operador de densidad si y solo si cumple con estas propiedades. Por esto, estos dos puntos funcionan como una definición alternativa al operador de densidad.
...
\begin{itemize}
    ...
    \item $\rho=\rho^{2}$\ddnote{cambié $n$ por $2$}.
    \item $\Tr(\rho^{2})=1$.
\end{itemize}
