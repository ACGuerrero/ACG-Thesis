...
\acnote{La siguiente sección ha sido iterada: reescritura. (viejo en iffalse)}
...
\acnote{---------------- Nuevo:}
\fi
Observamos entonces que hay una diferencia fundamental entre los sistemas que pueden ser descritos por un vector de estado y aquellos que no. Considérese el caso en que, dada la expresión \ref{eq:DensOpMix}, el estado del sistema es $\ket{\varphi_{1}}$ \ddnote{lo cambié por el ket, para que quede entendido que un estado puro es describible con un ket} con  probabilidad $p_{1}=1$, i.e. $\rho=\dyad{\varphi_{1}}$. En tal caso decimos que $\rho$ es un estado puro. Claramente, $\rho^{2}=\rho$. Esto hace de $\rho$ un proyector\ddnote{de rango $1$, de otra forma no se cumple que la traza sea $1$, por ej la matriz identidad es un proyector de rango completo}, de lo que se sigue que $\Tr(\rho^{1})=1$. 

Como en general se cumple que \ddnote*{$\Tr(\rho^{2})=\sum_i p^2_i\leq 1$}{$\Tr(\rho^{2})\leq 1$}, definimos a la pureza como una medida de que tan puro es un estado como \cite{Jaeger}
\begin{equation}
    \text{Pu}(\rho)=\Tr(\rho^{2}).\nonumber
\end{equation}
...
\end{equation}
Si un estado puede escribirse como un producto tensorial de estados pertenecientes a los subsistemas del sistema multipartito, entonces se dice que es un estado \textit{producto} o \textit{separable}. Nótese que, en general, los estados del sistema compuesto no son estados separables. En realidad, dadas \ddnote*{las bases están formadas por kets, no solo por los simbolos dentro de los kets. Para claridad, por fas pon las bases como kets}{$\{\varphi_{i}^{A}\}$ y $\{\varphi_{j}^{B}\}$} bases ortonormales de los espacios $\hilbert^{A}$ y $\hilbert^{B}$ respectivamente, podemos escribir a todo estado puro $\ket{\psi^{C}}$ del sistema multipartito como
\begin{equation}
...
\acnote{El siguiente párrafo ha sido iterado: notas}
\ddnote*{Esta pregunta está algo rebuscada, se puede simplificar, tipo: que sucede si solo nos es relevante el estado de una partícula?}{¿Qué sucede si al científico no le interesa sino uno de los subsistemas?} ¿Qué sucede si únicamente es relevante una de las dos partículas? Es en este caso en el que surge el concepto de la matriz de densidad reducida. Si $\rho^{C}$ es la matriz de densidad del sistema compuesto por $A$ y $B$, entonces la matriz de densidad reducida del sistema $A$ es
\begin{equation}
    \rho^{A}=\Tr_{B}(\rho^{C}),\nonumber
\end{equation}
...
\end{equation}
Puede verse que el resultado de la operación es trazar sobre los elementos del sistema que no es de interés. La matriz reducida del sistema $A$, o traza parcial con respecto al sistema $B$, actúa como matriz de densidad de $A$, ya que contiene toda la descripción estadística de dicho subsistema. \ddnote{En particular obsérvese lo siguiente, sea $G^A$ una observable del subsistema $A$, \ie{} $G^A$ actúa en $\rho^C$ como $(G^A\otimes \Id_B)$, se tiene entonces que
$$ \langle G^A \rangle =\tr\left((G^A\otimes \Id_B)\rho^C\right)=\tr \left( G^A \rho^A \right).$$
}
\ddnote{De pronto pon esto con tus palabras si no te gusta como está escrito.}
...
Considérese, en cambio, que el sistema de interés $\rho_{S}$ se halla acoplado a un entorno $\rho_{E}$ y que, inicialmente, el conjunto de estos dos conforma un sistema cerrado descrito por el operador de densidad $\rho(0)=\rho_{S}(0)\otimes\rho_{E}$. Esta condición inicial está justificada experimentalmente \ddnote*{si se considera que el sistema $S$ se prepara al hacer una medición proyectiva, de tal manera que el estado total queda separable}{: una medición proyectiva sobre el sistema de interés proyecta al sistema a un estado factorizable, así que estos estados son siempre preparables.}. \ddnote*{Dado que el}{Como el sistema} \ddnote {total} es cerrado, este evoluciona siguiendo la ecuación de Liouville-von Neumann,
\begin{align}
...
\end{align}
cuya solución formal está dada en términos de un \ddnote*{superoperador parametrizado por $t$,}{operador} $\mcE_{t}$,
\begin{equation}
...
En esta ecuación, $\mcE_{t}$ está definido como
 ...
 \end{equation}
 \ddnote{cambié un poco esta ecuación}
 
 y cumple que es un canal cuántico \ddnote{[cita], puedes citar el libro de Mario Ziman y el Nielsen and Chuang} \acnote{también llamado aplicación dinámica \cite{Breuer} o operación cuántica \cite{Chuang}} cuántico $\forall t$, y que \ddnote*{Aguas, esto es incorrecto, $\mcE_0$ no es la matriz identidad en $S$ al tiempo cero. Lo correcto es $\mcE_0=\text{id}_S$}{$\mcE_{0}=\Id_{S}$}. El formalismo de los canales cuánticos queda fuera del alcance de este trabajo, y sin entrar en más detalle, basta con señalar que los canales cuánticos son aplicaciones lineales completamente positivas que preservan la traza, \ie, todo canal cuántico $\mcE:\mcB(\hilbert_{A})\rightarrow \mcB(\hilbert_{B})$ cumple que \cite{Watrous}
 \ddnote{Lo que sigue está escrito raro, se presta a confusiones, propongo correcctión:}
 \begin{enumerate}
    \item \ddnote*{$\mcE\otimes \text{id}_{k}: \mcB(\hilbert_A \otimes \hilbert_k)\to \mcB(\hilbert_B \otimes \hilbert_k)$ es positivo para todo entero positivo $k$.}{$\mcE\otimes \Id_{\mcB(\hilbert_{C})}$ es positivo para cualquier $\hilbert_{C}$.}
...
\end{equation}
donde $\rho\in\mcS(\hilbert)$ y $\ket{e}\in \tilde \hilbert$. \ddnote{Hice varios cambios aquí, cuando el canal opera entre diferentes espacios de Hilbert, el teorema es ligeramente mas complicado. Mejor simplifiqué a cuando se usa el mismo espacio.}
\iffalse
...
Es común escoger alguna base hermítica para poder parametrizar a las matrices de densidad. El beneficio de hacer esto es que, por ser $\mcS(\hilbert_{n})$ un subconjunto de $\mcL(\hilbert_{n})$, dicha parametrización será \ddnote*{real}{lineal}, y aún más: los parámetros serán cantidades \ddnote*{promedios de observables}{medibles en el laboratorio} \ddnote{ojo aquí, los promedios no son directamente medibles, son computables, como lo son también los parámetros complejos}. \ddnote*{Propongo mejor esto: Dicho de otro modo, se puede escoger una base de tal forma que es un conjunto de observables tomográficamente completo.}{Esto significa que, realizando mediciones de forma adecuada, es posible reconstruir el estado de un sistema.} Una elección común de base son las matrices generalizadas de Gell-Mann, junto a la matriz identidad.
\iffalse
\acnote{Remplaza:-------}
...
Sea $\{\varsigma_{k}\}_{k}$ el conjunto de matrices generalizadas de Gell-Mann que generan a $\text{SU}(n)$ y
%En efecto, sea $\{\varsigma_{k}\}$ un conjunto de generadores de $\text{SU}(n)$ y 
$\rho$ una matriz de densidad $\rho\in\mcS(\hilbert_{n})$. Entonces $\rho$ está completamente descrita por el vector generalizado de Bloch de dimensión $n^{2}-1$, $\vec{\gamma}$ definido como \ddnote{En lo que sigue defines $\rho$ en terminos de $\vec \gamma$, tu necesitas definir $\vec \gamma$ en terminos de $\rho$, es decir, poner los componentes de $\vec \gamma$ en terminos del producto punto de HIlbert Schmidt. Y luego poner, tal que (y aquí ya pones la ecuación que está a continuación)}
