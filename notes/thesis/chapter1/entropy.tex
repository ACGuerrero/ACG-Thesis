...
\begin{enumerate}
    ...
    \item Si $X$ e $Y$ son procesos independientes, \ddnote*{$H(p_{X}(x_{j})p_{Y}(y_{k}))=H(p_{X}(x_{j}))+H(p_{Y}(y_{k}))$}{$H(p_{X}(x_{j}),p_{Y}(y_{k}))=H(p_{X}(x_{j}))+H(p_{Y}(y_{k}))$}.
\end{enumerate}
Además, demostró que
...
\begin{enumerate}
    ...
    \item La entropía de un estado producto es igual a la suma de las entropías de cada factor, $S(\rho_{A}\otimes\rho_{B})=S(\rho_{A})+S(\rho_{B})$. \ddnote{Esta propiedad es análoga al caso clásico en el que se tienen dos variables aleatorias independientes.}
\end{enumerate}