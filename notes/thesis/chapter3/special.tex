...
Conforme se aumenta el número de partículas, la expresión de la dinámica efectiva se vuelve cada vez más complicada y menos informativa. La dinámica unitaria general para el modelo de Ising cerrado en fase ordenada es
\begin{equation}
    U= \Id \cos^{n}(\omega t)+(-\rmi)^{n}\sin^{n}(\omega t) +\sum_{k=2}^{n-1}(-\rmi)^{n-k}\cos^{k}(\omega t)\sin^{n-k}(\omega t)\sum_{k_{1}<k_{2}}(\pauli{3,k_{1}}\pauli{3,k_{1}+1})(\pauli{3,k_{2}}\pauli{3,k_{2}+1}) 
\end{equation}
\subsubsection{Modelo XXX de Heisemberg}
