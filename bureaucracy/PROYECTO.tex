...
Considérese, pues, \textbf{que se aplica esta aplicación}\notaAd{nope} borrosa a un sistema de dos partículas caracterizado por un operador de densidad $\rho\in D(\Complex^{2}\otimes\Complex^{2})$, con $D(\Complex^{2}\otimes\Complex^{2})$ el espacio de los operadores de densidad que actúan sobre $\Complex^{2}\otimes\Complex^{2}$. Esta acción puede escribirse como
\begin{gather}
