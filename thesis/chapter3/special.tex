...
\end{equation}\ddnote{$\mcP$}
\ddnote{Usaste esta forma para algún calculo?}
...
\begin{equation}
    ...
    \end{pmatrix}.\nonumber
\end{equation}
Para extender la noción de canal de desfasamiento a $n$ qubits, primero nótese que dada la base de $\hilbert_{2}$ conformada por los eigenestados de $\sigma{3}$, $\{\ket{e_{0}},\ket{e_{1}}\}$, es posible construir una base $\{e_{\vec{k}}\}_{\vec{k}}$ \ddnote{falta notación de ket} de $\hilbert_{2^{n}}$ como
\begin{equation}
    \{e_{\vec{k}}\}_{\vec{k}}=\left\{\ket{e_{\vec{k}}}\in\hilbert_{2^{n}}: \ket{e_{\vec{k}}}=\Motimes_{j=1}^{n}\ket{e_{k_{j}}},\,k_{j}\in\{0,1\}\right\} \rlap{.}\nonumber
\end{equation}\ddnote{falta notación de ket}
...
\end{gather}\ddnote{$\mcP$}
que corresponde al canal de Pauli \ref{eq:PauliChannelN} cuando $\vec{j}=0$ \ddnote{estás igualando un vector a un número, aquí que hongo?}, y el segundo definido sobre la base de productos tensoriales de eigenestados de $\pauli{1}$,
\begin{gather}
...
\begin{equation}
    ...
    \end{pmatrix}.\nonumber
\end{equation}
donde se ha hecho $e^{\mu t}=\gamma$. Es claro que el canal de amortiguamiento y el canal de estabilización tienen efectos diferentes en las fases del estado. Aún más, mientras que la extensión del canal de estabilización a $n$ partículas es directa, la generalización del canal de amortiguamiento de amplitud es no trivial \ddnote{, ya que no hay una forma única de conectar los amortiguamientos entre los niveles energéticos}. \acnote{agregar cita.} \ddnote{Alguna vez encontré una, pero ya no la encuentro, de momento demosle poca prioridad a esta cita.}
