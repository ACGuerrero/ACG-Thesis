...
\subsubsection{\ddnote*{Dinámica local simétrica}{Dinámica simétrica}}
...
\end{equation}
\ddnote{explica por fas los subindices np} El \ddnote{estado del} conjunto de partículas no preferenciales es entonces
\begin{equation}
    \Motimes_{k=2}^{n}\rho_{k}=\rho_{\text{np}}^{\otimes(n-1)}\nonumber.
\end{equation}
Si estas no varían \ddnote{poner dinámica explicita}, mientras que el subsistema de interés se propaga de forma unitaria, entonces la dinámica efectiva es
\begin{equation}
...
\begin{figure}[ht!]
    ...
    \begin{subfigure}{0.32\textwidth}
      ...
      \caption{$t=0.5$}
    \end{subfigure}
    \caption{Efecto de la evolución subyacente sobre la esfera de Bloch si $r_{\ef}=0.9$, $p_{1}=0.6$ y $U=e^{-i\pi t \pauli{3}}$. La dramática contracción a lo largo de $z$ se asocia al alto valor de $1-p_{1}$.\ddnote{Se necesita caption casi-autocontenido. Hace falta poner para que dinámica es esto.}}
...
\end{align}
\ddnote{Nótese que en este regimen, la dinámica si es lineal y tiene operadores de Kraus $1/\sqrt{2}\left\{\Id, e^{\rmi\omega t \pauli{3}} \right\}$}
...
\end{equation}
\ddnote*{Sin perdida de generalidad, }{Por simplicidad,} tómese $H_{k}=\pauli{3}\,\forall k$. Recordando la ecuación (\ref{eq:rhoArhoB}), los valores de expectación de $\pauli{j}$ serán
\begin{align}
...
\begin{figure}[ht!]
    ...
    \end{subfigure}
    \caption{Oscilaciones periódicas cercanas al valor esperado. La periodicidad no depende de los pesos probabilísticos de cada partícula. En azul, el conjunto de estados con la misma pureza (y la misma coordenada en $\pauli{3}$) que el estado efectivo inicial (en verde).\ddnote{De nuez, se necesita caption mas autocontenido, que sistea y que estado inicial.}}\label{fig:OscilationsSameHam}
\end{figure}
...
\begin{figure}[ht!]
    ...
    \end{subfigure}
    \caption{Variaciones cercanas al estado efectivo inicial (verde). El aumento en el número de partículas lleva a una evolución complicada y no periódica, pero que converge al punto $\vec{r}_{1}$. Valores menos importantes de $p_{1}$ conducen a mayores variaciones. En azul, el conjunto de estados con la misma pureza y la misma coordenada en $\pauli{3}$ que el estado efectivo inicial. \ddnote{autocontenido}}\label{fig:PrefInv2}
\end{figure}

\subsubsection{Sistema en campo magnético ? \ddnote{Partículas no interactuantes con diferente frecuencia de transición}}

\acnote{No tengo idea de qué justificación se pueda dar a que todas las partículas oscilen en la misma dirección pero con frecuencias diferentes.} \ddnote{Esta y las dos dinámicas anteriores son casos particulares de partículas no interactuantes y ya, que tan facil sea su realización, pues no sé.}
...
\begin{figure}[ht!]
    ...
    \end{subfigure}
    \caption{Como mencionado, para el caso en que $r_{\ef}=0.9$ y $p=0.95$ las variaciones debidas al error no son perceptibles debido a su pequeña amplitud, independientemente del número de partículas consideradas. \ddnote{autocontenido}}\label{fig:Oscilations12}
\end{figure}
\begin{figure}[ht!]
    \centering
    \begin{subfigure}{0.5\textwidth}
    ...
    \end{subfigure}
    \caption{Si $r_{\ef}=0.9$ y $p=0.3$, el alejamiento de la evolución esperada se hace notar. Para valores grandes de $n$ se vuelve dominante el término de $p_{1}$ \ddnote{autocontenido. Otro comentario, no sé si has tenido problemas con las referencias a los labels de las figuras, pero los labels van dentro del caption para que funcionen bien, en este caso ya lo metí al caption, por fas ve el tex}
\label{fig:Oscilations13}    
}
\end{figure}
